...
## D4: localOnly は config pinning で表現

- 状況: 「ローカルPCのみ実行対象」の解釈が幅広い。
- 決定: config を PC 名にピン留めし、他PCでは起動しない/コマンドを無効化。
- 影響:
  - config をコピーしても別PCで誤実行しにくい。
  - PC 名変更時の扱いは要検討（ピン留め解除/更新フローが必要）。
